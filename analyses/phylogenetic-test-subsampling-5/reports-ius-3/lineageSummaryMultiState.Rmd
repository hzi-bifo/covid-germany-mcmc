...
```{r lineage-treemap, fig.width=7, fig.height=2.0, fig.cap="Partition of `r nrow(metadata)` `r states` genomes into `r states` transmission lineages and singletons, coloured by duration of lineage detection (time between the lineage’s oldest and most recent genomes). (Transmission lineages from the MCC trees)."}
  ...
  for (s in stateInfo$stateOrder) { #1:nrow(stateFiles)
    ...
    print(kable(data %>% arrange(-seqs) %>% top_n(10) %>% dplyr::select(cluster) %>% left_join(clusterStatsMCC, by="cluster") %>% dplyr::select(cluster, seqs, tmrca_shifted_calendar, tmrca_calendar) %>%
      ...
      print(kable(data %>% filter(seqs >= 8) %>% arrange(tmrca_shifted_calendar) %>% top_n(100) %>% dplyr::select(cluster) %>% left_join(clusterStatsMCC, by="cluster") %>% dplyr::select(cluster, seqs, tmrca_shifted_calendar, tmrca_calendar) %>%
      left_join(clusterSamplesMCC %>% filter(Collection.date >= as.Date("2021-01-01")) %>% group_by(cluster) %>% dplyr::summarise(seqs.after.jan = n()), by = "cluster")))
