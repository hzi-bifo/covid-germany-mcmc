...
for (cluster in 1:length(germanyLargeClusters)) {
  x <- barplot2(aggs.matrix[,cluster], col=cols[cluster], las=2, ylim=c(0,4000))
...
breaks <- seq(as.Date("2020-01-01"), as.Date("2021-05-01"), by="week")

for (lin in levels(as.factor(clusterStatsMCC$pangolin))) {
  # clusterStatsMCC %>% group_by(pangolin) %>% summarise()
  
  
  dateFreqDistribution(hist((clusterStatsMCC %>% 
          filter(pangolin == lin & tmrca_calendar >= breaks[1] & tmrca_calendar < breaks[length(breaks)]))$tmrca_calendar, 
       plot=FALSE, breaks=breaks)$counts, 
       breaks, plot.ci=FALSE,  barplot=FALSE, 
                         startDate = breaks[1], 
                         endDate = breaks[length(breaks)-1], 
                         col=cols, border = "#000000", 
                         # ymax = max(rowSums(weeklyBreakdown$clustSizes)) + 2, 
                         ylab = "No. imported lineages",
       main = paste("TMRCA of pango lineage ", lin))
