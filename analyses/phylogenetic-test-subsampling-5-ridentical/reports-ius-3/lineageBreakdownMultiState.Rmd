...
lineages <- c("B.1.1.7_MCC_619", "B.1.1.7_MCC_558", "B.1.1.7_MCC_588", "B.1.1.7_MCC_600", "B.1.1.7_MCC_511", "B.1.1.7_MCC_549", "B.1.177_MCC_39", "B.1.1.7_MCC_584", "B.1.1.7_MCC_360")
...
for (s in 1:nrow(stateFiles)) {
  ...
  plotLineageDurations(clusterStatsMCC %>% left_join(clusterSamplesMCC %>% filter(Collection.date >= as.Date("2021-01-01")) %>% group_by(cluster) %>% dplyr::summarise(seq.after.jan = n()), by = "cluster") %>% filter(seq.after.jan >= 50) %>% arrange(-seqs), clusterSamplesMCC, startDate="2020-06-01", endDate="2021-06-01", cutoff=cutoff, orderby = "", state=state, pal = NULL, main = "Lineages with at least 8 samples after 2021-01")
}
```
