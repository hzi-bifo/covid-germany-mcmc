...
#     state <- stateFiles$state[s]
#     clusterStatsMCC <- stateInfo$clusterStatsMCC[[s]] 
#     clusterStatsMCC.f <- clusterStatsMCC %>% filter(cluster %in% (clusterSamplesMCC %>% filter(Collection.date >= as.Date("2020-07-01")) %>% pull(cluster) ))
#     
# 
...
#     for (i in 1:length(tmrcaStat)) {
# 
#         tmrca_hist_single<- hist(clusterSamplesMCC[clusterSamplesMCC$lineage_seqs == 1 & clusterSamplesMCC[,tmrcaStat[i]] >= tmrcaBreaks[1] & clusterSamplesMCC[,tmrcaStat[i]] <= tmrcaBreaks[length(tmrcaBreaks)-1]  & clusterSamplesMCC$Collection.date >= as.Date("2020-07-01"), tmrcaStat[i]], breaks=tmrcaBreaks, plot=FALSE, right=FALSE)   
#         tmrca_hist_small <- hist(clusterStatsMCC.f[clusterStatsMCC$seqs <= 1, tmrcaStat[i]], breaks=tmrcaBreaks, plot=FALSE, right=FALSE)   
#         tmrca_hist_med   <- hist(clusterStatsMCC.f[clusterStatsMCC$seqs > 1  & clusterStatsMCC$seqs <= 5, tmrcaStat[i]], breaks=tmrcaBreaks, plot=FALSE, right=FALSE)   
#         tmrca_hist_big   <- hist(clusterStatsMCC.f[clusterStatsMCC$seqs > 5 & clusterStatsMCC$seqs <= 10, tmrcaStat[i]], breaks=tmrcaBreaks, plot=FALSE, right=FALSE)   
#         tmrca_hist_huge  <- hist(clusterStatsMCC.f[clusterStatsMCC$seqs > 10, tmrcaStat[i]], breaks=tmrcaBreaks, plot=FALSE, right=FALSE)   
#         
#         # Stacked barplot
#         tmrca_hist_breakdown <- data.frame(huge  = tmrca_hist_huge$counts,
#                                            big   = tmrca_hist_big$counts,
...
#         dateFreqDistribution(t(tmrca_hist_breakdown), tmrcaBreaks, plot.ci=FALSE, barplot=TRUE, label=LETTERS[i], ylab=ylabs[i],
#                              #startDate = startDate, endDate = plotEnd, 
#                          startDate = tmrcaBreaks[26], endDate = tmrcaBreaks[length(tmrcaBreaks)-1], 
# 				col=mPal(unlist(ukPal), 0.75), border=mPal(unlist(ukPal)), ymax=40)
...
#         legend("top", horiz=FALSE, inset=c(0,-0.33), bty='n', xpd=TRUE, ncol=2,
#                fill=mPal(unlist(ukPal, 0.75)), border = mPal(unlist(ukPal)), 
#                legend = c("Bigger than 10", "5 to 10", "2 to 4", "1 or smaller"), title = paste("Importation lineage size of", state),
#                cex=0.8)
#         
#     }
#   }
...
```{r tmrca-shift-comparison-sizes-weekly-by-case, fig.width=7, fig.height=6, fig.cap="(A) Histogram of lineage TMRCAs, coloured by lineage size. (B) Histogram of lineage importations, coloured by lineage size.", eval=TRUE}
...
#     # tmrcaBreaks <- seq.Date(as.Date("2019-01-01"), as.Date("2021-06-20"), by="7 days")
#     # tmrcaBreaks <- seq.Date(min(clusterStatsMCC$tmrca_shifted_calendar), max(clusterStatsMCC$tmrca_calendar)+7, by="7 days")
#     tmrcaBreaks <- seq.Date(min(as.Date(clusterStatsMCC$tmrca_shifted_calendar)), max(as.Date(clusterSamplesMCC$Collection.date))+7, by="7 days")
# 
#     layout(matrix(1:2, nrow=2, byrow=TRUE))
#     par(mar=c(4,6,2.5,6), cex.axis=0.7, cex.lab=0.8, cex.main=1.5, mgp=c(3,0.75,0))
# 
...
#     # plotEnd <- as.Date("2020-06-21")
#     ylabs <- c("Frequency of TMRCAs\n(per week)", "Frequency of importations\n(per week)")
#     data <- clusterSamplesMCC %>% filter(Collection.date >= as.Date("2020-07-01")) %>% group_by(cluster) %>% dplyr::summarise(tmrca_calendar = min(tmrca_calendar), tmrca_shifted_calendar = min(tmrca_shifted_calendar), seqs = n())
#     
#     for (i in 1:length(tmrcaStat)) {
# 
#         # tmrca_hist_single<- hist(clusterSamplesMCC[clusterSamplesMCC$lineage_seqs == 1 & clusterSamplesMCC[,tmrcaStat[i]] >= tmrcaBreaks[1] & clusterSamplesMCC[,tmrcaStat[i]] <= tmrcaBreaks[length(tmrcaBreaks)-1]  & clusterSamplesMCC$Collection.date >= as.Date("2020-07-01"), tmrcaStat[i]], breaks=tmrcaBreaks, plot=FALSE, right=FALSE)   
...
#         # tmrca_hist_huge  <- hist(clusterStatsMCC.f[clusterStatsMCC$seqs > 10, tmrcaStat[i]], breaks=tmrcaBreaks, plot=FALSE, right=FALSE)   
#         
#         tmrca_hist_small <- hist((data %>% filter(seqs == 1) %>% as.data.frame())[, tmrcaStat[i]], breaks=tmrcaBreaks, plot=FALSE, right=FALSE)
#         tmrca_hist_med  <- hist((data %>% filter(seqs > 1, seqs <= 5) %>% as.data.frame())[, tmrcaStat[i]], breaks=tmrcaBreaks, plot=FALSE, right=FALSE)
#         tmrca_hist_big    <- hist((data %>% filter(seqs > 5, seqs <= 10) %>% as.data.frame())[, tmrcaStat[i]], breaks=tmrcaBreaks, plot=FALSE, right=FALSE)
#         tmrca_hist_huge    <- hist((data %>% filter(seqs > 10) %>% as.data.frame())[, tmrcaStat[i]], breaks=tmrcaBreaks, plot=FALSE, right=FALSE)
#         # Stacked barplot
#         tmrca_hist_breakdown <- data.frame(huge  = tmrca_hist_huge$counts,
#                                            big   = tmrca_hist_big$counts,
#                                            med   = tmrca_hist_med$counts,
...
```{r, fig.width=7, fig.height=4, eval=TRUE}
   ...
   clusterSamplesMCC <-
      clusterSamplesMCC %>% 
     # left_join(clusterStatsMCC %>% dplyr::select(cluster, tmrca_calendar, tmrca_shifted_calendar), by=c("cluster"="cluster")) %>%
      mutate(tmrca_calendar = if_else(is.na(tmrca_calendar), oldest_calendar, tmrca_calendar),
...
clusterFirstCasesWithSingles <- clusterFirstCasesWithSingles %>% mutate(weight = 1/cluster.first.case.count) %>% dplyr::select(cluster, lineage_seqs, weight, adm1, tmrca_calendar, tmrca_shifted_calendar)

size_category <- function(lineage_seqs) {
  return(if_else(lineage_seqs <= 1, "small", if_else(lineage_seqs <= 5, "med", if_else(lineage_seqs <= 10, "big", "huge"))))
}

for (adm1_ in list(c("Bavaria"), c("Baden-Wurttemberg"), c("North Rhine-Westphalia"), c("Saxony"),
                   clusterSamplesMCC %>% pull(adm1) %>% as.factor() %>% levels(), 
                   c("Berlin", "Brandenburg", "Bremen", "Hamburg", "Hesse", "Lower Saxony", "Mecklenburg-Western Pomerania", "Rhineland-Palatinate", "Saarland", "Saxony-Anhalt", "Schleswig-Holstein", "Thuringia")) ) {
  
  tmrcaStat <- c("tmrca_calendar", "tmrca_shifted_calendar")
  ylabs <- c("Frequency of TMRCAs\n(per week)", "Frequency of importations\n(per week)")
  data <- clusterFirstCasesWithSingles %>% filter(adm1 %in% adm1_)
  
  
  # layout(matrix(1:2, nrow=2, byrow=TRUE))
  par(mar=c(6,6,4,6), cex.axis=0.7, cex.lab=0.8, cex.main=1.5, mgp=c(3,0.75,0))
  
  for (i in 1:length(tmrcaStat)) {
    data2 <- data %>% mutate(tmrca.break.index = sapply(.[[tmrcaStat[i]]], function(x) which.max(x < tmrcaBreaks) - 1)) %>%
      mutate(sz = size_category(lineage_seqs)) %>% group_by(tmrca.break.index, sz) %>% dplyr::summarise(n = sum(weight)) %>%
      bind_rows(crossing(tmrca.break.index = seq(length(tmrcaBreaks)-1), sz=c("small", "med", "big", "huge"), n=0)) %>%
      group_by(tmrca.break.index, sz) %>% dplyr::summarise(n = sum(n)) %>% arrange(tmrca.break.index) %>% filter(tmrca.break.index >= 1)
    
    tmrca_hist_breakdown <- data.frame(huge  = data2 %>% filter(sz == "huge") %>% pull(n),
                                       big   = data2 %>% filter(sz == "big") %>% pull(n),
                                       med   = data2 %>% filter(sz == "med") %>% pull(n),
                                       small = data2 %>% filter(sz == "small") %>% pull(n)
    )
    dateFreqDistribution(t(tmrca_hist_breakdown), tmrcaBreaks, plot.ci=FALSE, barplot=TRUE, label=LETTERS[i], ylab=ylabs[i],
                         #startDate = startDate, endDate = plotEnd, 
                         startDate = tmrcaBreaks[26], endDate = tmrcaBreaks[length(tmrcaBreaks)-1], 
    ...
    legend("top", horiz=FALSE, inset=c(0,-0.33), bty='n', xpd=TRUE, ncol=2,
           fill=mPal(unlist(ukPal, 0.75)), border = mPal(unlist(ukPal)), 
           legend = c("Bigger than 10", "5 to 10", "2 to 4", "1 or smaller"), title = paste("Importation lineage size of", paste(adm1_, collapse = ' '), "\n", tmrcaStat[i]),
           cex=0.8)
    
    print(kable(cbind(date=tmrcaBreaks[1:(length(tmrcaBreaks)-1)], tmrca_hist_breakdown %>% mutate(Total = huge+big+med+small)), caption = paste("Lineage", tmrcaStat[i], paste(adm1_, collapse = ", ") )))
...
tmrcaBreaks <- seq.Date(as.Date("2020-01-16") - 14, max(data$tmrca_shifted_calendar) + 14, by="week")

  data <- clusterSamplesMCC %>% group_by(cluster, tmrca_shifted_calendar, tmrca_calendar, lineage_seqs) %>% dplyr::summarise(n=n()) %>% mutate(pangolin = gsub("_.*", "", cluster))

  for (i in 1:length(tmrcaStat)) {
    data2 <- data %>% mutate(tmrca.break.index = sapply(.[[tmrcaStat[i]]], function(x) which.max(x < tmrcaBreaks) - 1), tmrca.break.value = tmrcaBreaks[tmrca.break.index]) 
    
    p <- ggplot(data2, 
           aes(x=tmrca.break.value, fill=pangolin)) + 
      #geom_histogram(bins = 30)
      geom_bar()+
      scale_x_date(date_breaks = "4 week", date_labels = "%d-%b")+
      guides(fill=guide_legend(ncol=2))+
      theme(axis.text.x = element_text(angle = 90)) + ggtitle(paste("Number of importations into Germany", tmrcaStat[i])) +
    ...
    print(p)
    print(kable(
      data2 %>% group_by(tmrca.break.value, pangolin) %>% dplyr::summarise(n = n()) %>% pivot_wider(names_from = pangolin, values_from = n, values_fill = 0),
      caption=paste("importation into germany", tmrcaStat[i])
    ))
  }
...
deStates <- clusterSamplesMCC$adm1 %>% as.factor() %>% levels()
   ...
   for (dvar in c('tmrca_shifted_calendar')) {
       ...
       tmrcaBreaks <- seq.Date(startDate, endDate, by="1 day")
         ...
         clusterFirstCaseByCountyWithSingles <-
            ...
            group_by(adm1, tmrca.break.index) %>%
            dplyr::summarise(n = sum(1 / cluster.first.case.count))
      smoothedCharts <- function(adm1_s, adm1_name, col, marks__, clusterFirstCase = clusterFirstCaseByCountyWithSingles, title_add = "") {
         data <- clusterFirstCase %>% filter(adm1 %in% adm1_s) %>% 
         ...
         print(kable(
           data, 
           caption = paste("Change rate of\n  smoothed importation frequency", adm1_name, "y=n.diff")
         ))
      ...
      for (adm1_ in adm1List) {
         ...
         smoothedCharts(adm1_, adm1_, pal[[adm1_]], marks__, clusterFirstCase = clusterFirstCaseByCounty, title_add = paste("\nno single", dvar))
      }
      
      marks_test = marks_; #for (x in names(marks_test)) { marks_test[[x]]$color = "gray"; }
      smoothedCharts(c(levels(factor((clusterFirstCaseByCounty %>% dplyr::select(adm1))$adm1)), NA), state, pal[[state]], marks_, title_add = paste("\nsingle", dvar))

      smoothedCharts(c(levels(factor((clusterFirstCaseByCounty %>% dplyr::select(adm1))$adm1)), NA), state, pal[[state]], marks_, clusterFirstCase = clusterFirstCaseByCounty, title_add = paste("\nno single", dvar))
      ...
      smoothedCharts(setdiff(c(levels(factor((clusterFirstCaseByCounty %>% dplyr::select(adm1))$adm1)), NA), c("Bavaria", "North Rhine-Westphalia", "Baden-Wurttemberg", "Saxony")), "other", pal[["Other"]], marks_test, clusterFirstCase = clusterFirstCaseByCounty, title_add = paste("\nno single", dvar))
      smoothedCharts(setdiff(c(levels(factor((clusterFirstCaseByCounty %>% dplyr::select(adm1))$adm1)), NA), c("Bavaria", "North Rhine-Westphalia", "Baden-Wurttemberg", "Saxony")), "other", pal[["Other"]], marks_test, clusterFirstCase = clusterFirstCaseByCountyWithSingles, title_add = paste("\nsingle", dvar))
   
      pal[["NRW,BV"]] <- pal[["Other"]]
      smoothedCharts(setdiff(c(levels(factor((clusterFirstCaseByCounty %>% dplyr::select(adm1))$adm1)), NA), c("North Rhine-Westphalia", "Bavaria")), "no NRW,BV", pal[["Other"]], marks_test, clusterFirstCase = clusterFirstCaseByCounty, title_add = paste("\nno single", dvar))

      pal[["no NRW,BV"]] <- pal[["Other"]]
      smoothedCharts(setdiff(c(levels(factor((clusterFirstCaseByCountyWithSingles %>% dplyr::select(adm1))$adm1)), NA), c("North Rhine-Westphalia", "Bavaria")), "no NRW,BV", pal[["Other"]], marks_test, clusterFirstCase = clusterFirstCaseByCountyWithSingles, title_add = paste("\nsingle", dvar))
   }
...
```{r effectiveness of NPIs (tmrca): importation-state-specific-10-day, fig.width=7, fig.height=4, eval=TRUE}


startDate <- as.Date("2020-09-25")
endDate <- as.Date("2021-05-24")


dvar_shift <- -5

   for (dvar in c('tmrca_calendar')) {
       ...
       tmrcaBreaks <- seq.Date(startDate, endDate, by="1 day")
         ...
         clusterFirstCaseByCounty <-
            clusterFirstCases %>% 
            # mutate(adm1 = ifelse(is.na(adm1) | adm1 == "NA", "Saxony", adm1)) %>%
            mutate(tmrca.break.index = sapply(.[[dvar]], function(x) which.max(x + dvar_shift < tmrcaBreaks) - 1)) %>%
            group_by(adm1, tmrca.break.index) %>%
            dplyr::summarise(n = sum(1 / cluster.first.case.count)) %>%
            mutate(p =  round(n / sum(n) * 100, digits = 1)) 
      ...
      smoothedCharts <- function(adm1_s, adm1_name, col, marks__) {
         data <- clusterFirstCaseByCountyWithSingles %>% filter(adm1 %in% adm1_s) %>% 
          ...
          plot_weekly_table(data, "adm1", pal, 
                         # paste("Imp conv to triangle (10 d)", "(+sing)", adm1_name, dvar), 
                         "", 
                         "", paste("Smoothed importation frequency of", adm1_name, dvar, dvar_shift),
                         legend_ = function(x) ifelse(nchar(x) > 10, substr(x, 1, 10), x), printTable_ = FALSE, marks_ = marks__ )
         ...
         p <- ggplot() + 
            ...
            # geom_text_repel(data=marks__extended, aes(label=label, x=date, y=smdiff.pref), color=marks__extended$color, nudge_y = 0.01 * ymax, vjust = 0, direction = "x", segment.curvature = -0.1, segment.linetype = 6, force = 0) +
            geom_text(data=marks__extended, aes(label=label, x=date, y=smdiff.pref), color=marks__extended$color, vjust = 0, nudge_y = 0.01 * ymax) +
            ylab(paste("Change rate of\n  smoothed importation frequency", adm1_name, dvar, dvar_shift)) + 
            theme(axis.title.x = element_blank())
         ...
         print(kable(
            ...
            caption=paste("Change in critical points", adm1_name, dvar, dvar_shift)))
      }
      ...
      smoothedCharts(setdiff(c(levels(factor((clusterFirstCaseByCounty %>% dplyr::select(adm1))$adm1)), NA), c("Bavaria", "North Rhine-Westphalia")), " no NRW,BV", pal[["Other"]], marks_test)
...
# 
# clusterFirstCasesWithSingles2 <-
#    clusterSamplesMCC %>% left_join(clusterSamplesMCC %>% group_by(cluster) %>% dplyr::summarise(oldest_calendar_2 = min(Collection.date))) %>%
#   filter(.data[["oldest_calendar_2"]] == Collection.date)
# 
# clusterFirstCasesCount2 <- clusterFirstCasesWithSingles2 %>% group_by(cluster) %>% dplyr::summarise(cluster.first.case.count = n())
# 
# clusterFirstCasesWithSingles2 <-
#    clusterFirstCasesWithSingles2 %>% left_join(clusterFirstCasesCount2, by = c("cluster" = "cluster"))
# 
# 
# clusterFirstCaseByCountyWithSingles <-
#    clusterFirstCasesWithSingles %>%
...
kable(data2 %>% group_by(adm1.y) %>% dplyr::arrange(-n, .by_group = TRUE) %>% dplyr::summarise(text =paste(adm1.x, paste0("(", n, ")"), collapse = ", ")), caption = "Number of importations to each state (row) from different source states.")
...
data6 <- data2 %>% filter(adm1.x != adm1.y) %>% left_join(stateLineageCount %>% dplyr::rename(adm1.y.lincount = n), by=c("adm1.y" = "adm1")) %>% 
  ...
  filter(n >= 1, n.ratio >= 0.0) %>% dplyr::rename(src=adm1.x, dst=adm1.y, move=n.ratio) %>% 
...
print(kable(data2 %>% pivot_wider(values_from = n, names_from = adm1.y, values_fill = 0), caption="no. of lineages observed in adm1.x not later than adm1.y (no extra condition)"))
print(kable(data2 %>% filter(adm1.x != adm1.y) %>% dplyr::group_by(adm1.x) %>% dplyr::summarise(lineage.state.pair.after = sum(n)) %>% arrange(-lineage.state.pair.after) %>%
              left_join(data2 %>% filter(adm1.x != adm1.y) %>% dplyr::group_by(adm1.x) %>% dplyr::summarise(state.after = n()) , by="adm1.x")
              , caption="no. of lineage/state pairs observed in other states not before than adm1, no. states with cluster observed after adm1"))

data6 <- data2 %>% filter(adm1.x != adm1.y) %>% left_join(stateLineageCount %>% dplyr::rename(adm1.y.lincount = n), by=c("adm1.y" = "adm1")) %>% 
  mutate(n.ratio = n/adm1.y.lincount) %>% 
  filter(n >= 5) %>% dplyr::rename(src=adm1.x, dst=adm1.y, move=n.ratio) %>% 
  left_join(germany.state.grography %>% dplyr::rename(src.id = id, src.lat = lat, src.long = long), by=c("src"="value")) %>% 
  left_join(germany.state.grography %>% dplyr::rename(dst.id = id, dst.lat = lat, dst.long = long), by=c("dst"="value")) %>%
  left_join(map.edge.info, by=c("src", "dst")) 
...
for (state in c("Baden-Wurttemberg", "Bavaria", "North Rhine-Westphalia", "Saxony")) {
