...
```{r rsetup, include=FALSE}
  ...
  # intreepath : "../results/beast/run/all/"
    ...
    # Calculating limit for each lineage    
    # suppressWarnings(
    #   lineageFirstCase <- metadata %>% select(Pango.lineage, decimal_date) %>% dplyr::filter(!is.na(Pango.lineage)) %>% group_by(Pango.lineage) %>% summarise(min.date = min(decimal_date, na.rm = TRUE)) %>% arrange(Pango.lineage) 
    # )
    # lineageLimit = sapply(treefiles, function (x) { y <- lineageFirstCase$min.date[lineageFirstCase$Pango.lineage == gsub("-.*", "", x)]; if (length(y) == 0) return(uklimit); return(y);} )
    ...
    for (treefile in treefiles) {
        ...
        for (cutoff in cutoffs) {
            ...
            #prefix    <- paste0(strsplit(treefile, "-")[[1]][1], "_",cluster_f,"_")
	          #prefix    <- paste0(gsub("gisaid-", paste0("LIN-", state, "-"), gsub(".tree", "", treefile)), "_",cluster_f,"_")
            ...
            # treetable <- getClusterTable(tree, state, prefix = prefix, cutoff = cutoff, maxheight = (mostrecent - uklimit), debug = FALSE) 
