...
clusterDFS <- function(tree, node, cluster, treetable, debug = FALSE) {
  ...
  if (node == 5934) {
    cat(paste("cDFS", node, parent, treetable$visited[parent], treetable$location[parent]))
  }
  
  if (parent > 0 && !treetable$visited[parent] && treetable$location[parent] == location ) { #&& treetable$sampled[parent]) {
  ...
  for (child in children) {
    if (!treetable$visited[child] && treetable$location[child] == location ) { #&& treetable$sampled[parent]) {
...
getClusterTable <- function(tree, location, prefix = "cluster_", cutoff=NA, maxheight=NA, debug = FALSE, metadata = NULL) {
  ...
  # if (!is.null(metadata)) {
  #   treetable$sampled   <- c(metadata[match(ttree$tip.label, metadata$Accession.ID), "is_sampled"], rep(NA, ttree$Nnode))  # Should be in order
  # } else {
  #   treetable$sampled   <- c(rep(TRUE, length(ttree$tip.label)), rep(NA, ttree$Nnode))  # Should be in order
  # }
  ...
  if (!is.na(maxheight)) {
      ...
      #treetable$location[treetable$types == "coalescent" & treetable$location == "Germany" & treetable$heights > maxheight] <- "nonGermany"
      treetable$location[treetable$types == "coalescent" & treetable$location == location & treetable$heights > maxheight] <- paste0("non", location)
  }
  ...
  for (i in 1:nrow(treetable)) {
      if (treetable$location[i] == location && is.na(treetable$cluster[i]) ) { #&& treetable$sampled[i]) {
          start.time <- Sys.time()
