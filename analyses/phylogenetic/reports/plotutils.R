...
dateFreqDistribution <- function(heights, dates, startDate="2020-01-01", endDate="2020-06-26", 
  ...
  if (!add) {
    plotShadedAxes(xlim=c(startDate, endDate), ylim=c(0, ymax), ylab=ylab, yaxs='i', xaxs='i', dateFormat = dateFormat, ...)
  }
...
#                                        plotNames=TRUE, plotDurations=TRUE, state="Germany", ...) {
#   
...
# 
#     notState <- paste("Not", state)
#     clustSamples <- clusterSamples[clusterSamples$cluster == as.character(clusterSummary$cluster[i]), ]
#     clustSamples$adm1                 <- sapply(strsplit(clustSamples$Location, '/'), function(x) ifelse(length(x) >= 3, str_trim(x[3]), notState))
#     # 
#     # clustSamples$adm1 <- as.character(clustSamples$adm1)    
#     clustSamples$adm1[clustSamples$country != state] <- notState
#     
...
plotLineageDurations <- function(clusterSummary, clusterSamples, orderby="mostrecent",
                                 startDate="2020-01-01", endDate="2020-06-26", cutoff="2019-01-01",
                                 plotNames=TRUE, plotDurations=TRUE, state = "Germany", pal = NULL, ncol = 1, text.width = NULL, 
                                 legend.inset = c(-1.3,0), legend.adm1.pos = "right", ...) {
  ...
  # colMap  <- list("Germany" = mPal(ukPal$eng),
  #                 "Not Germany" = mPal(ukPal$sct))
  # 
  # 
  # fillMap <- list("Germany" = mPal(ukPal$eng, 0.5),
  #                 "Not Germany" = mPal(ukPal$sct, 0.5))
  # 
  # labelMap <- c("Germany")
  ...
  notstate <- paste0("Non ", state)
  
  # colMap  <- list( mPal(ukPal$eng),
  #                 mPal(ukPal$sct))
  # names(colMap) <- c(state, notstate)
  colMap <- lapply(pal, function(x) mPal(x, 0.5))
  
  # fillMap <- list(mPal(ukPal$eng, 0.5),
  #                 mPal(ukPal$sct, 0.5))
  # names(fillMap) <- c(state, notstate)
  fillMap <- lapply(pal, function(x) mPal(x, 0.5))
  
  # labelMap <- c(names(dePal))
  labelMap <- c()
  ...
  for (i in plotOrder) {
    ...
    clustSamples <- clusterSamples[clusterSamples$cluster == as.character(clusterSummary$cluster[i]), ]
    # #clustSamples$adm1 <- as.character(clustSamples$adm1)        
    # clustSamples$adm1 <- as.character(clustSamples$country)
    # #clustSamples$adm1[clustSamples$country != "UK"] <- "Not UK"
    # clustSamples$adm1[clustSamples$adm1 != "Germany"] <- "Not Germany"
    
    # cat(paste(clustSamples$Location))
    # clustSamples$adm1 <- state
    if (!("adm1" %in% colnames(clustSamples))) {
      clustSamples$adm1 <- sapply(
        str_split(paste0(clustSamples$Location,"/",clustSamples$Additional.location.information), "/"), function(x) str_trim(x[3]) )
      clustSamples$adm1[is.na(clustSamples$adm1)] <- notstate
      clustSamples$adm1[clustSamples$adm1 == "NA"] <- notstate
      clustSamples$adm1[!clustSamples$instate] <- notstate
    }
    
    labelMap <- c(labelMap, clustSamples$adm1)
    
    # cat(paste(clustSamples$adm1))
    ...
    samplesX <- clustSamples$decimal_date[clustSamples$decimal_date >= cutoff]
    samplesY <- jitter(rep(j, length(samplesX)), amount=0.15)
    fills    <- sapply(clustSamples$adm1, function(x) if (x %in% names(fillMap)) fillMap[[x]] else stop(paste0("'", x, "'") ))
    points(samplesX, samplesY, col=fills, pch=16, cex=0.5)
  ...
  labelMap <- unique(labelMap)
  labelMapFill <- unlist(lapply(labelMap, function(x) {d <- c(fillMap[[x]]); names(d) = x; return(d);}))
  legend(legend.adm1.pos, legend=labelMap, bty='n', inset=legend.inset, xpd=TRUE,
         col=labelMapFill, pch=16, text.col = "#FFFFFFFF", cex=par("cex.lab"), ncol=ncol, text.width = text.width)
  legend(legend.adm1.pos, legend=labelMap, bty='n', inset=legend.inset, xpd=TRUE,
         col=labelMapFill, pch=1, cex=par("cex.lab"), ncol=ncol, text.width = text.width)
  legend("top", horiz=TRUE, legend=c("TMRCA"), bty='n', inset=c(0,-0.05), xpd=TRUE,
         pch=c(8), pt.cex=0.8, cex=par("cex.lab"))
  # legend("top", labelMap, xpd=TRUE, 
  #        fill=unlist(lapply(labelMap, function(x) {d <- c(fillMap[[x]]); names(d) = x; return(d);})), ncol=2)
  # legend("top", horiz=TRUE, legend=labelMap, bty='n', inset=c(0,-0.08), xpd=TRUE,
  #        col=unlist(colMap), pch=1, cex=par("cex.lab"))
  # legend("top", horiz=TRUE, legend=c("TMRCA"), bty='n', inset=c(0,-0.05), xpd=TRUE,
  #        pch=c(8), pt.cex=0.8, cex=par("cex.lab"))
...
legend.map.germany <- function(legend.position = "topright", 
                               legend.inset = c(-0.40, 0), legend.ncol = 2, legend.map.color = dePal, 
                               legend.map.viewport = c(2,30,5,1)) {
