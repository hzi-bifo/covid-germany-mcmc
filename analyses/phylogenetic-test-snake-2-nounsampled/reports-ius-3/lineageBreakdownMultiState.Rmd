...
for (s in stateInfo$stateOrder) { 
 ...
 #  }
  ...
  #   geom_line() + 
  #   scale_x_date(date_labels="%b %d", date_breaks = "7 day", minor_breaks = "1 day", limits = c(as.Date("2020-10-01"), as.Date("2021-05-27")), expand = c(0, 0)) +
  #   scale_color_manual(values = unlist(lineagePal[colnames(weeklyBreakdown$clustSizesTop5)][1:8]), name = "Lineage") +
  ...
  # print(p)
  # 
  tmrcaBreaks <- seq(startDate, endDate, by="2 week")

  data <- clusterSamplesMCC %>% filter(cluster %in% (clusterStatsMCC %>% top_n(n=10, wt = seqs) %>% pull(cluster))) %>% mutate(tmrca.break.index = sapply(Collection.date, function(x) which.max(x < tmrcaBreaks) - 1), tmrca.break.value = tmrcaBreaks[tmrca.break.index]) %>% group_by(cluster, tmrca.break.index, tmrca.break.value) %>% dplyr::summarise(n = n(), lineage_seqs = first(lineage_seqs)) %>% ungroup() %>% dplyr::select(-tmrca.break.index, -lineage_seqs) %>% rbind(data.frame(cluster=NA, tmrca.break.value = tmrcaBreaks, n = 0))  %>% complete(tmrca.break.value, cluster) %>% mutate(n = ifelse(is.na(n), 0, n)) %>% filter(!is.na(cluster))
  
  clusters <- data %>% pull(cluster) %>% as.factor() %>% levels()
  cols <- scale_color_brewer(palette = "Spectral")$palette(n = length(clusters))
  cols <- rainbow(length(clusters))
  names(cols) <- clusters
  
  for (cls in clusters) {
    
   p <- ggplot(data %>% filter(cluster == cls), aes(x=tmrca.break.value, y=n, color = cluster)) +
    geom_line() + 
    scale_x_date(date_labels="%b %d", date_breaks = "14 day", minor_breaks = "1 day", limits = c(as.Date("2020-09-01"), as.Date("2021-05-27")), expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0), limits=c(0, 20)) + 
    # scale_color_manual(values = darken(rainbow(20), amount = 0.0)) +
   ...
   print(p + scale_x_date(date_labels="%b %d", date_breaks = "14 day", minor_breaks = "1 day", limits = c(as.Date("2020-08-01"), as.Date("2021-05-27")), expand = c(0, 0)))
  }
    
  # p <- ggplot(data, aes(x=tmrca.break.value, y=n, color = cluster)) +
  #   geom_line() + 
  #   scale_x_date(date_labels="%b %d", date_breaks = "7 day", minor_breaks = "1 day", limits = c(as.Date("2020-10-01"), as.Date("2021-05-27")), expand = c(0, 0)) +
  ...
  # print(p)
   ...
   p <- ggplot(data, aes(x=tmrca.break.value, y=n, color = cluster)) +
    geom_line() + 
    scale_x_date(date_labels="%b %d", date_breaks = "14 day", minor_breaks = "1 day", limits = c(as.Date("2020-08-01"), as.Date("2021-05-27")), expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0), limits=c(0, 20)) + 
    # scale_color_manual(values = darken(rainbow(20), amount = 0.0)) +
     # scale_color_brewer(palette = "Spectral") + 
     scale_color_manual(values = cols) +
...
```
```{r sample-breakdown-weekly-proportion-ntech, fig.width=7, fig.height=3, fig.cap = sample_breakdown_weekly_proportion_cap, eval=TRUE}
tmrcaBreaks <- seq(startDate, endDate, by="2 week")

data <- clusterSamplesMCC %>% filter(cluster %in% (clusterStatsMCC %>% top_n(n=10, wt = seqs) %>% pull(cluster))) %>% mutate(tmrca.break.index = sapply(Collection.date, function(x) which.max(x < tmrcaBreaks) - 1), tmrca.break.value = tmrcaBreaks[tmrca.break.index]) %>% group_by(cluster, tmrca.break.index, tmrca.break.value) %>% dplyr::summarise(n = n(), lineage_seqs = first(lineage_seqs)) %>% ungroup() %>% dplyr::select(-tmrca.break.index, -lineage_seqs) %>% rbind(data.frame(cluster=NA, tmrca.break.value = tmrcaBreaks, n = 0))  %>% 
  left_join(clusterSamplesMCC %>% mutate(tmrca.break.index = sapply(Collection.date, function(x) which.max(x < tmrcaBreaks) - 1), tmrca.break.value = tmrcaBreaks[tmrca.break.index]) %>% group_by(tmrca.break.index, tmrca.break.value) %>% dplyr::summarise(total = n()) %>% ungroup() %>% dplyr::select(-tmrca.break.index), by="tmrca.break.value") %>%
  complete(tmrca.break.value, cluster) %>% mutate(n = ifelse(is.na(n), 0, n), total = ifelse(is.na(total), 0, total)) %>% filter(!is.na(cluster)) %>%
  mutate(ratio = ifelse(total == 0, 0, n/total))
  ...
  for (cls in clusters) {
   ...
   p <- ggplot(data %>% filter(cluster == cls), aes(x=tmrca.break.value, y=ratio, color = cluster, fill=cluster)) +
    geom_area() + 
    scale_x_date(date_labels="%b %d", date_breaks = "14 day", minor_breaks = "1 day", limits = c(as.Date("2020-09-01"), as.Date("2021-05-27")), expand = c(0, 0)) +
  ...
  # print(p)
   ...
   p <- ggplot(data, aes(x=tmrca.break.value, y=ratio, fill = cluster, color=cluster)) +
    # geom_line() + 
...
# cols <- extend_to(unlist(lineagePal), length(germanyLargeClusters))
cols <- color[1:length(germanyLargeClusters)]
names(cols) <- germanyLargeClusters
cols[intersect(germanyLargeClusters, names(lineagePal))] = unlist(lineagePal)[intersect(germanyLargeClusters, names(lineagePal))]
...
clusterSamplesMCC <- clusterSamplesMCC %>% mutate(adm1 = ifelse(adm1 == "Baden-Württemberg", "Baden-Wurttemberg", adm1))
...
for (clusterIndex in 1:length(germanyLargeClusters)) {
  ...
  data <- clusterSamplesMCC %>% filter(cluster == largeCluster, Collection.date >= dataStartDate) %>% group_by(adm1) %>% dplyr::summarise(c.n = n()) %>% join(dataClusterAdm1SampleCount, by="adm1") %>% mutate(c.n.ratio = c.n/n * 100) %>% filter(adm1 != "NA")
   ...
   plot_on_map(data$adm1, data$c.n.ratio, 
            title=paste0("Proportion of sequences from\n", largeCluster), legend_title = "Percentage", col = "#154E80", 
            max_value = 12.0, legend.position = "bottom")
...
for (s in 1:nrow(stateFiles)) {
  ...
  # par(mar=c(5,10,3,7), cex.axis=0.7, cex.lab=0.8, mgp=c(2,0.75,0))
  plotLineageDurations(largeLineages %>% filter(seqs >= 8), clusterSamplesMCC, startDate="2020-06-01", endDate="2021-06-01", cutoff=cutoff, orderby = "", state=state, pal = NULL)
...
data <- clusterSamplesMCC %>% mutate(adm1=ifelse(adm1=="Baden-Württemberg", "Baden-Wurttemberg", adm1)) %>% filter(as.Date(Collection.date) >= as.Date("2021-02-01")) %>% mutate(week = calculateWeek(as.Date(sample_date), referenceDate)) %>% dplyr::group_by(adm1, cluster) %>% dplyr::summarise(seq = n()) %>% ungroup() 
...
data <- clusterSamplesMCC %>% mutate(week = calculateWeek(as.Date(sample_date), referenceDate)) %>% dplyr::group_by(week, adm1, cluster) %>% dplyr::summarise(seq = n()) %>% ungroup() 
data <- data %>% filter(adm1 %in% countyImportant) %>% 
  dplyr::union(data %>% group_by(week, cluster) %>% dplyr::summarise(seq = n()) %>% ungroup() %>% mutate(adm1 = "Germany")) %>% 
  dplyr::union(data %>% filter(!adm1 %in% countyImportant, adm1 != "NA") %>% group_by(week, cluster) %>% dplyr::summarise(seq = n()) %>% ungroup() %>% mutate(adm1 = "Other")) %>% 
...
print(kable(data %>% filter(adm1!="NA") %>% left_join(clusterSamplesMCC %>% filter(Collection.date >= as.Date("2021-01-01")) %>% group_by(adm1) %>% dplyr::summarise(seq.after.2021 = n())) %>% left_join(clusterSamplesMCC %>% group_by(adm1) %>% dplyr::summarise(seq = n())) %>% arrange(-species.evenness), 
            caption = paste("Number of analyzed sequences after 01-Jan-2021", "Lineage evenness", "Shannon index")))
...
print(kable(
  data %>% left_join(clusterSamplesMCC %>% filter(Collection.date >= as.Date("2021-01-01")) %>% group_by(adm1) %>% dplyr::summarise(lin.no = n_distinct(cluster))) %>% filter(adm1 != "NA"), 
  caption = paste("Number of importation lineages since 01-Jan-2021", "Lineage evenness")
))
...
print(kable(
  clusterSamplesMCC %>% filter(Collection.date >= as.Date("2021-01-01")) %>% group_by(adm1) %>% dplyr::summarise(lin.no = n_distinct(cluster), seq = n()) %>% filter(adm1 != "NA"),
  caption = paste("Number of sequences since 01-Jan-2021", "Number of sequences since 01-Jan-2021")
))
...
statesAllGermanyI <- c(statesAllGermany, "Germany")

for (adm1.index in seq(length(statesAllGermanyI))) {
  adm1.i = statesAllGermanyI[adm1.index]
  ...
  treemap(data %>% filter(adm1 == adm1.i) %>% filter(seq > 0) %>% mutate(old = as.numeric(as.Date("2021-06-01") - tmrca_shifted_calendar)), index="cluster", vSize = "seq", vColor = "old", type="value", 
          title = adm1.i, title.legend = paste0("Importation date (days before 2021-06-1)"), 
            fontsize.labels = 10, border.lwds = 0.4, cex=2, fontsize.legend=9, position.legend = "bottom", range=c(0,700), mapping=c(0,as.numeric(as.Date("2021-06-01"))-as.numeric(as.Date("2021-01-01")),700), palette="RdYlBu")
}
...
# "hellinger", "aitchison", "morisita",
for (d in c("manhattan", "euclidean", "canberra", "clark", "bray", "kulczynski", "jaccard", "gower", "altGower",  "horn", "mountford", "raup", "binomial", "chao", "cao", "mahalanobis", "chisq", "chord", "robust.aitchison")) {
  cat(d)
...
```{r first-sequence, fig.width=6, fig.height=6, eval=TRUE}
...
plot_on_map(clusterFirstCaseByCounty$adm1[clusterFirstCaseByCounty$adm1 != "NA"], clusterFirstCaseByCounty$p[clusterFirstCaseByCounty$adm1 != "NA"], title="D", legend_title = "Percentage", col = "#d44077", max_value = max(clusterFirstCaseByCounty$p[clusterFirstCaseByCounty$adm1 != "NA"]), legend.position = "bottom", text.font.size = NULL)

plot_on_map(clusterFirstCaseByCounty$adm1[clusterFirstCaseByCounty$adm1 != "NA"], clusterFirstCaseByCounty$p[clusterFirstCaseByCounty$adm1 != "NA"], title="", legend_title = "Percentage", col = "#d44077", max_value = max(clusterFirstCaseByCounty$p[clusterFirstCaseByCounty$adm1 != "NA"]), legend.position = "bottom", text.font.size = NULL)
...
plot_on_map(clusterFirstCaseByCountyNoSingleton$adm1[clusterFirstCaseByCountyNoSingleton$adm1 != "NA"], clusterFirstCaseByCountyNoSingleton$p[clusterFirstCaseByCountyNoSingleton$adm1 != "NA"], title="no singleton", legend_title = "Percentage", col = "#d44077", max_value = max(clusterFirstCaseByCountyNoSingleton$p[clusterFirstCaseByCountyNoSingleton$adm1 != "NA"]), legend.position = "bottom", text.font.size = NULL)

```


```{r first-sequence-with-single, fig.width=6, fig.height=6, eval=TRUE}

kable(clusterFirstCasesWithSingles %>% group_by(cluster) %>% dplyr::summarise(n = n()) %>% arrange(-n) %>% filter(n > 1) %>% left_join(clusterStatsMCC, by=c("cluster" = "cluster")) %>% dplyr::rename(no.first.case = n), caption="Clusters with more than one first case +single")

kable(clusterFirstCaseByCountyWithSingles %>% group_by(adm1) %>% dplyr::summarise(n = sum(n)) %>% full_join(deCounty %>% dplyr::select(state), by=c("adm1" = "state")) %>% mutate(n = ifelse(is.na(n), 0, n)) %>% arrange(-n), caption = "Number of first cases in each state +single")
...
# #title="oldest seqs of importation lineages"
# plot_on_map(clusterFirstCaseByCountyWithSingles$adm1[clusterFirstCaseByCountyWithSingles$adm1 != "NA"], clusterFirstCaseByCountyWithSingles$p[clusterFirstCaseByCountyWithSingles$adm1 != "NA"], title="D", legend_title = "Percentage", col = "#d44077", max_value = max(clusterFirstCaseByCountyWithSingles$p[clusterFirstCaseByCountyWithSingles$adm1 != "NA"]), legend.position = "bottom", text.font.size = NULL)
# 
# plot_on_map(clusterFirstCaseByCountyWithSingles$adm1[clusterFirstCaseByCountyWithSingles$adm1 != "NA"], clusterFirstCaseByCountyWithSingles$p[clusterFirstCaseByCounty$adm1 != "NA"], title="", legend_title = "Percentage", col = "#d44077", max_value = max(clusterFirstCaseByCountyWithSingles$p[clusterFirstCaseByCountyWithSingles$adm1 != "NA"]), legend.position = "bottom", text.font.size = NULL)
# 
# # plot_on_map(clusterFirstCaseByCounty %>% group_by(adm1) %>% dplyr::summarise(n = sum(n)) %>% full_join(deCounty %>% dplyr::select(state), by=c("adm1" = "state")) %>% mutate(n = ifelse(is.na(n), 0, n)) %>% arrange(-n) %>% filter(adm1 != "NA") %>% pull(adm1), clusterFirstCaseByCounty %>% group_by(adm1) %>% dplyr::summarise(n = sum(n)) %>% full_join(deCounty %>% dplyr::select(state), by=c("adm1" = "state")) %>% mutate(n = ifelse(is.na(n), 0, n), p=n/sum(n)) %>% arrange(-n) %>% filter(adm1 != "NA") %>% pull(p), title="oldest seqs of importation lineages", legend_title = "Percent", col = "#d7191c", max_value = max(clusterFirstCaseByCounty$p[clusterFirstCaseByCounty$adm1 != "NA"]))
...
# x <- clusterSamplesMCC %>% group_by(cluster) %>% mutate(lineage_oldest2 = min(decimal_date)) %>% ungroup()

clusterFirstCases <- clusterSamplesMCC %>% mutate(lineage_oldest = ifelse(is.na(lineage_oldest), decimal_date, lineage_oldest)) %>% 
  filter(gsub("_.*", "", cluster) == lin & lineage_seqs > lsm & abs(lineage_oldest - decimal_date) < 1/366 + firstCaseDistance/366) 
clusterFirstCasesCount <- clusterFirstCases %>% group_by(cluster) %>% dplyr::summarise(cluster.first.case.count = n())
clusterFirstCases <- clusterFirstCases %>% left_join(clusterFirstCasesCount, by=c("cluster" = "cluster"))
clusterFirstCaseByCounty <- clusterFirstCases %>% filter(cluster.first.case.count <= 1000000) %>% group_by(adm1) %>% 
  dplyr::summarise(n = sum(1/cluster.first.case.count)) %>% 
  rbind(data.frame(adm1=clusterSamplesMCC %>% pull(adm1) %>% as.factor() %>% levels(), n=0)) %>%
  dplyr::group_by(adm1) %>% dplyr::summarise(n=sum(n)) %>% 
  mutate(p =  round(n/sum(n) * 100, digits=1)) %>% arrange(desc(n)) 
...
```{r empty-map}
plot_on_map(clusterFirstCaseByCounty$adm1[clusterFirstCaseByCounty$adm1 != "NA"], clusterFirstCaseByCounty %>% filter(adm1 != "NA") %>% mutate(x=0) %>% pull(x), 
            title="", legend_title = "Percentage", col = "#d44077", max_value = max(clusterFirstCaseByCounty$p[clusterFirstCaseByCounty$adm1 != "NA"]), 
            legend.position = "none", text.font.size = NULL)
