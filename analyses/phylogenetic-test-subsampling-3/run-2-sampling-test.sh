...
cd /net/viral_genomics/covid-lineage/huge-lineage-dynamics/analyses/phylogenetic-test-subsampling-3/
DATE_TREE=20210602
...
./scripts/sample-evenly --in results/gisaid-$DATE_TREE-$STATE.tree --out results/gisaid-$DATE_TREE-sampled.tree~ -l $STATE non$STATE --samples-out results/gisaid-$DATE_TREE-samples.txt~ --metadata results/gisaid-$DATE_METADATA-metadata.tsv --bucket-size 100 1 1 --special 5 $SUB_TREES --keep 0 10000 $SUB_TREES --unbias-method ratio 
...
cd $DIR/
cd results/fasttree/$DATE_TREE-$X/ ;
bash $DIR/sarscov2phylo/global_tree_gisaid.sh -i ./gisaid-$DATE_SEQ-$X-seq0.fa -o gisaid-$DATE_SEQ-ft -t 52
#conda deactivate sarscovphylo
cd $DIR
rm results/gisaid-$DATE_TREE-$X-1.tree
...
for X in $SUB_TREES; do 
for i in {31..35}; do 
	qsub -cwd -N beast-$X-$i -M foroughmand@gmail.com -l h_vmem=10G,mem_free=10G,s_vmem=10G -pe smp 3 -o results/beast/run/out/$X-$i -e results/beast/run/out/$X-$i run-beast-st.sh run/$X-$i $DATE_TREE $X
	#JOB_ID=$RANDOM
	#bash run-beast-st.sh run/$X-$i $DATE_TREE $X 2>results/beast/run/out/$X-$i/beast-$X-$i.e$JOB_ID >results/beast/run/out/$X-$i/beast-$X-$i.o$JOB_ID
done
done
...
#Following might be corrected now after re-execution of everyting from the start
## B.1.1.519 removed due to small size beast could not be executed
#SUB_TREES="A B.1.1.7 B.1.1.70 B.1.1.317 B.1.177 B.1.160 B.1.221 B.1.36 B.1.258 B.1.351 C"
...
#bash run-beast-DTA-st.sh run/$X-$i $DATE_TREE $X 2>results/beast/run/out/$X-$i/DTA-$X-$i.e$PID >results/beast/run/out/$X-$i/DTA-$X-$i.o$PID 
done
done
...
for X in $SUB_TREES; do
	echo "Tree $X "
	# Only allow identicals to be matched
	#   Output folder: temp/beast/run/lin-ius-3/
	mkdir -p temp/beast/run/lin-ius-3/out/ temp/beast/unsampled/
	scripts/lineage-importation-inject-unsampled --tree results/beast/unsampled/$X-DTA-$DATE_TREE.MCC.tree.2.nexus --dist <( zcat results/beast/run/lin-ius/alignment-distance-$DATE_TREE-$X-id.txt.gz ) -l \"$STATE\" \"non$STATE\" --out-folder temp/beast/run/lin-ius-3/ --metadata results/gisaid-$DATE_TREE-metadata-sampled-unsampled.tsv --lin-prefix LIN-Germany-$X-"$DATE_TREE"_DTA_MCC_ --treefile temp/beast/unsampled/$X-DTA-$DATE_TREE.MCC.tree.nexus --out-clusters temp/beast/run/lin-ius-3/out/$X-clusters_DTA_MCC_NA.tsv --out-cluster-samples temp/beast/run/lin-ius-3/out/$X-clusterSamples_DTA_MCC_NA.tsv --out-clusters-single temp/beast/run/lin-ius-3/out/$X-clusters_DTA_MCC_singles.tsv --out-0.5 temp/beast/run/lin-ius-3/out/$X-clusters_DTA_MCC_0.5.tsv temp/beast/run/lin-ius-3/out/$X-clusterSamples_DTA_MCC_0.5.tsv temp/beast/run/lin-ius-3/out/$X-clusters_DTA_MCC_singles_0.5.tsv --allow-new-lineage false --dist-threshold 0.0
done

mkdir -p temp/lin-ius-3/out temp/beast/unsampled/
for X in $SUB_TREES; do
scripts/lineage-importation-inject-unsampled --tree results/beast/unsampled/$X-DTA-$DATE_TREE.MCC.tree.2.nexus --dist <( zcat results/beast/run/lin-ius/alignment-distance-$DATE_TREE-$X-id.txt.gz ) -l \"$STATE\" \"non$STATE\" --out-folder temp/lin-ius-3/ --metadata results/gisaid-$DATE_TREE-metadata-sampled-unsampled.tsv --lin-prefix LIN-Germany-$X-"$DATE_TREE"_DTA_MCC_ --treefile temp/beast/unsampled/$X-DTA-$DATE_TREE.MCC.tree.nexus --out-clusters temp/lin-ius-3/out/$X-clusters_DTA_MCC_NA.tsv --out-cluster-samples temp/lin-ius-3/out/$X-clusterSamples_DTA_MCC_NA.tsv --out-clusters-single temp/lin-ius-3/out/$X-clusters_DTA_MCC_singles.tsv --out-0.5 temp/lin-ius-3/out/$X-clusters_DTA_MCC_0.5.tsv temp/lin-ius-3/out/$X-clusterSamples_DTA_MCC_0.5.tsv temp/lin-ius-3/out/$X-clusters_DTA_MCC_singles_0.5.tsv --allow-new-lineage false --dist-threshold 0.0 --log temp/lin-ius-3/$X-2.log --do-assign true
done
...
cp ../phylogenetic-test-snake-2/data/X-DTAmulti-template.xml data/
cp ../phylogenetic-test-snake-2/run-beast-DTAmulti.sh .
...
for X in $SUB_TREES; do
	echo "Tree $X "
	#scripts/lineage-importation-inject-unsampled --tree results/dtamulti/$X-DTA-$DATE_TREE.MCC.tree.2.nexus --dist <( cat /dev/null ) -l \"$STATE\" \"non$STATE\" --out-folder temp/beast/run/lin-ius-3/ --metadata results/gisaid-$DATE_TREE-metadata-sampled-unsampled.tsv --lin-prefix LIN-Germany-$X-"$DATE_TREE"_DTA_MCC_ --treefile temp/beast/unsampled/$X-DTA-$DATE_TREE.MCC.tree.nexus --out-clusters temp/beast/run/lin-ius-3/out/$X-clusters_DTA_MCC_NA.tsv --out-cluster-samples temp/beast/run/lin-ius-3/out/$X-clusterSamples_DTA_MCC_NA.tsv --out-clusters-single temp/beast/run/lin-ius-3/out/$X-clusters_DTA_MCC_singles.tsv --out-0.5 temp/beast/run/lin-ius-3/out/$X-clusters_DTA_MCC_0.5.tsv temp/beast/run/lin-ius-3/out/$X-clusterSamples_DTA_MCC_0.5.tsv temp/beast/run/lin-ius-3/out/$X-clusters_DTA_MCC_singles_0.5.tsv --allow-new-lineage false --dist-threshold 0.0
	scripts/lineage-importation-inject-unsampled --tree results/beast/unsampled/$X-DTA-$DATE_TREE.MCC.tree.2.nexus --dist <( cat /dev/null ) -l \"$STATE\" \"non$STATE\" --out-folder temp/beast/run/lin-ius-3/ --metadata results/gisaid-$DATE_TREE-metadata-sampled-unsampled.tsv --lin-prefix LIN-Germany-$X-"$DATE_TREE"_DTA_MCC_ --treefile temp/beast/unsampled/$X-DTA-$DATE_TREE.MCC.tree.nexus --out-clusters temp/beast/run/lin-ius-3/out/$X-clusters_DTA_MCC_NA.tsv --out-cluster-samples temp/beast/run/lin-ius-3/out/$X-clusterSamples_DTA_MCC_NA.tsv --out-clusters-single temp/beast/run/lin-ius-3/out/$X-clusters_DTA_MCC_singles.tsv --out-0.5 temp/beast/run/lin-ius-3/out/$X-clusters_DTA_MCC_0.5.tsv temp/beast/run/lin-ius-3/out/$X-clusterSamples_DTA_MCC_0.5.tsv temp/beast/run/lin-ius-3/out/$X-clusters_DTA_MCC_singles_0.5.tsv --allow-new-lineage false --dist-threshold 0.0
done

for X in $SUB_TREES; do cat results/beast/run/lin-ius-3/out/$X-clusterSamples_DTA_MCC_0.5.tsv; done > x
for X in $SUB_TREES; do cat temp/beast/run/lin-ius-3/out/$X-clusterSamples_DTA_MCC_0.5.tsv; done > x
./scripts/lineage-importation-extract-multidta --tree results/dtamulti/run/all/sampled-DTA-$DATE_TREE.MCC.nexus --lineage-samples x --out results/dtamulti/run/lin-ius-3/clusterMovement_DTA_MCC_0.5.0.tsv -l \"Baden-Wurttemberg\" \"Bavaria\" \"Berlin\" \"Brandenburg\" \"Bremen\" \"Hamburg\" \"Hesse\" \"Lower Saxony\" \"Mecklenburg-Western Pomerania\" \"North Rhine-Westphalia\" \"Rhineland-Palatinate\" \"Saarland\" \"Saxony\" \"Saxony-Anhalt\" \"Schleswig-Holstein\" \"Thuringia\" --root-date 1900
