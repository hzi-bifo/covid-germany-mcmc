...
```{r sample-histogram, fig.width=7, fig.height=3, fig.cap = "Collection dates of the `nrow(metadata)` genomes analysed here (left-hand axis). Genomes are coloured by sampling location."}
  ...
  axis(side=4, las=2)
   ...
   # plotLogAxis(lim=c(1, ymax), side=4)
   mtext(side=4, text="Cases and genomes", line=3, cex=0.8)
   lines(deCases$date, average_(deCases$case), lwd=1, col=dePal[["all"]])
...
```{r case-by-county, fig.width=10 fig.height=4}
...
```
```{r case-by-county-all-in-one-weekly, fig.width=10, fig.height=4}
...
# library(weights)
sampleBreaks <- seq.Date(deCases$date[1], deCases$date[length(deCases$date)]+7, by="month")
...
plot_on_map(rownames(data), data, "Sequencing rate", "rate", col = "#00CC00") 
...
data <- deCountyDaily %>% dplyr::filter(date >= as.Date("2020-10-01")) %>% group_by(county) %>% dplyr::summarise(n = sum(case)) %>% filter(county != "NA")

plot_on_map(data$county, data$n, paste("No. confirmed cases since 2020-10-01 to", endDate) , "Count", col = "#FFFF00") 
