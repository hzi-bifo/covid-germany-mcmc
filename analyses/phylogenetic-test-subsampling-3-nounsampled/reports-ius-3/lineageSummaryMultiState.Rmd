...
subtitle: "Transmission Lineage Summary for Hamburg"
...
```{r rsetup, include=FALSE}
  ...
  # outputpath : "../results/trees-gisaid-Hamburg-20210417-lin/"
  # state      : "Hamburg"
    ...
    figpath    <- paste0(outputfolder, "figures/lineage_summary_figures_", params$device, "/")
    dir.create(figpath, recursive = TRUE, showWarnings = FALSE)
    
    cachepath  <- paste0(outputfolder, "figures/cache/lineageSummary_", params$device, "/")
...
# Summary
This notebook plots summary statistics and figures of the German transmission lineages extracted from the GISAID tree and Sankoff algorithm analyses.
...
```{r load-data, cache=TRUE}
   ...
   if (beta == 0) {
     clusterStatsMCC$tmrca_shifted_calendar <- clusterStatsMCC$tmrca_calendar - alpha
     clusterStatsMCC$tmrca_shifted <- as.Date(date_decimal(clusterStatsMCC$tmrca_shifted_calendar))
...
#    nsingles  <- sapply(seq_len(nreps), function(i) sum(metadata$country == "Germany") - sum(clusterStats$seqs[clusterStats$tree == i]))
...
## Input data
- Transmission lineages and singletons, on the dataset, n = `r nrow(metadata)` sequences (n = `r rsumstate(function(state, i, clusterStatsMCC) sum(stateInfo$metadata_instate[,i]))`, `r rsumstate(function(state, i, clusterStatsMCC) 100*round(sum(stateInfo$metadata_instate[,i])/nrow(metadata),2) )`% from the `r state``).





```{r imports-exports, fig.width=7, fig.height=3, fig.cap="Number of location state transitions between the binary phylogenetic traits `r state`/non-`r state` detected by the robust counting approach implemented in BEAST 1.10. Non-'r state' to `r state`=blue, `r state` to non-`r state`=red. Posterior distributions are truncated at their 95% HPD interval limits and the horizontal lines indicate median estimates."}
...
#    singletons    <- data.frame("Total" = sapply(seq_len(nreps), function(i) sum(metadata$country == "Germany") - sum(clusterStats$seqs[clusterStats$tree == i])))
#     singletonsMCC <- data.frame("Total" = sum(metadata$country == "Germany") - sum(clusterStatsMCC$seqs))
...
# #        singletons[[lin]]    <- sapply(seq_len(nreps), function(i) sum(treemeta$country == "Germany") - sum(clusterStats$seqs[clusterStats$tree == i & clusterStats$treefile == treefile]))
#         singletonsMCC[[lin]] <- sum(treemeta$country == "Germany") - sum(clusterStatsMCC$seqs[clusterStatsMCC$treefile == gsub("trees", "MCC.tree", treefile)])
#     }
#     
#     toc()
...
#    			#importCount     = importCountHPD[plotOrder, "med"],
...
#    
#    colnames(importTable) <- c(
#    			#"Non-UK to UK state transitions (median and 95% HPD)", "", 
...
```{r lineage-treemap, fig.width=12, fig.height=3, fig.cap="Partition of `r nrow(metadata)` `r states` genomes into `r states` transmission lineages and singletons, coloured by duration of lineage detection (time between the lineage’s oldest and most recent genomes). (Transmission lineages from the MCC trees)."}

  
  # for (s in stateInfo$stateOrder) { #1:nrow(stateFiles)
  s <- stateInfo$stateOrder[1]
    state <- stateFiles$state[s]
    ...
    gradPal <- rev(heat.colors(n = 408))

    #treemap(clustersAndSingletons, index=c("cluster"), vSize = "seqs", vColor = "duration", type="manual", palette = gradPal, range = c(0, 408),
         #   title = state, title.legend = "Transmission lineage duration (days)", 
        #    fontsize.labels = 0, border.lwds = 0.5, cex=1, fontsize.legend=10, position.legend = "bottom")
    
    
    pangolineagePal = unlist(pangoPal)
    # pangolineagePal = brewer.pal(n=length(pangoPal), name='RdBu')
    # names(pangolineagePal) <- names(pangoPal)
    data <- clusterStatsMCC %>% filter(mostrecent >= decimal_date(as.Date("2021-01-01"))) %>%  
      dplyr::select(cluster, seqs) %>% 
      rbind(clusterSamplesMCC %>% filter(lineage_seqs == 1) %>% dplyr::select(cluster) %>% mutate(seqs=1)) %>%
      mutate(pangolin = as.factor(gsub("_.*", "", cluster))) %>% 
      mutate(label = ifelse(cluster %in% (clusterStatsMCC %>%  top_n(100, seqs) %>% pull(cluster)), cluster, ""))
      # mutate(label = ifelse(cluster %in% (data %>%  top_n(5, seqs) %>% pull(cluster)), cluster, ""))
      # mutate(color = pangolineagePal[pangolin])
    


    treemap(data, index=c("cluster", "label"), vSize = "seqs", vColor = "pangolin", type="categorical", palette = pangolineagePal[levels(data$pangolin)],
            fontsize.labels=c(0,10), 
            title = "", title.legend = "",  border.lwds = 0.4, cex=0, fontsize.legend=8, position.legend = "right")
    # legend("top", horiz = FALSE, inset = c(0, -0.33), bty = 'n',
    #        xpd = TRUE, ncol = 6, fill = mPal(unlist(pangolineagePal), 0.75),
    #        border = mPal(unlist(pangolineagePal)), legend = names(pangolineagePal),
    #        title = paste("Transmission lineage of pangolineage", "(no singleton)"), cex = 0.8)

    data <- clusterStatsMCC %>% 
      # filter(mostrecent >= decimal_date(as.Date("2021-01-01"))) %>%  
      dplyr::select(cluster, seqs) %>% 
      rbind(clusterSamplesMCC %>% filter(lineage_seqs == 1) %>% dplyr::select(cluster) %>% mutate(seqs=1)) %>%
      mutate(pangolin = as.factor(gsub("_.*", "", cluster))) %>% 
      mutate(label = ifelse(cluster %in% (clusterStatsMCC %>%  top_n(100, seqs) %>% pull(cluster)), cluster, ""))
    ...
    treemap(data, 
            # index=c("cluster", "label"), 
            index="cluster",
            vSize = "seqs", vColor = "pangolin", type="categorical", palette = pangolineagePal[levels(data$pangolin)],
            fontsize.labels=c(0,10), 
            title = "+before 01.01.2021", title.legend = "",  border.lwds = 0.4, cex=0, fontsize.legend=8, position.legend = "right")
    
    print(kable(data %>% arrange(-seqs) %>% top_n(30, seqs) %>% dplyr::select(cluster) %>% 
                  left_join(clusterStatsMCC, by="cluster") %>% dplyr::select(cluster, seqs, tmrca_shifted_calendar, tmrca_calendar) %>%
                  left_join(clusterSamplesMCC %>% filter(Collection.date >= as.Date("2021-01-01")) %>% group_by(cluster) %>% dplyr::summarise(seqs.after.jan = n()), by = "cluster"), caption = "+befire 01.01.2021" ))

        # plot.window(c(0, 1), c(0, 1), xaxs="i")

    data <- clusterStatsMCC %>% filter(mostrecent >= decimal_date(as.Date("2021-01-01"))) %>%  
      dplyr::select(cluster, seqs, tmrca_shifted_calendar, tmrca_calendar) %>% 
      # rbind(clusterSamplesMCC %>% filter(lineage_seqs == 1) %>% dplyr::select(cluster) %>% mutate(seqs=1)) %>%
      mutate(pangolin = as.factor(gsub("_.*", "", cluster))) %>% 
      mutate(label = ifelse(cluster %in% (clusterStatsMCC %>%  top_n(100, seqs) %>% pull(cluster)), cluster, "")) 
      # mutate(label = ifelse(cluster %in% (data %>%  top_n(5, seqs) %>% pull(cluster)), cluster, ""))
    ...
    treemap(data, 
            ...
            vSize = "seqs", vColor = "pangolin", type="categorical", palette = pangolineagePal[levels(data$pangolin)],
            fontsize.labels=c(0,10), 
            title = "no single", title.legend = "",  border.lwds = 0.4, cex=0, fontsize.legend=8, position.legend = "right")
    
    print(kable(data %>% arrange(-seqs) %>% top_n(30, seqs) %>% dplyr::select(cluster) %>% left_join(clusterStatsMCC, by="cluster") %>% dplyr::select(cluster, seqs, tmrca_shifted_calendar, tmrca_calendar) %>%
      left_join(clusterSamplesMCC %>% filter(Collection.date >= as.Date("2021-01-01")) %>% group_by(cluster) %>% dplyr::summarise(seqs.after.jan = n()), by = "cluster") ))

      print(kable(data %>% filter(seqs >= 6) %>% arrange(tmrca_shifted_calendar) %>% top_n(30, seqs) %>% dplyr::select(cluster) %>% left_join(clusterStatsMCC, by="cluster") %>% dplyr::select(cluster, seqs, tmrca_shifted_calendar, tmrca_calendar) %>%
      left_join(clusterSamplesMCC %>% filter(Collection.date >= as.Date("2021-01-01")) %>% group_by(cluster) %>% dplyr::summarise(seqs.after.jan = n()), by = "cluster")))
...
```{r lineage-sizes-data, cache=TRUE}
  ...
  stateInfo$sizeBreaks <- stateInfo$sizeFreqDistMCC <- list()
  ...
  for (s in 1:nrow(stateFiles)) { #
    ...
    sizeBreaks  <- seq(1, max(clusterStatsMCC$seqs)+9, 10)
    stateInfo$n <- 60
    n <- stateInfo$n
...
# #    
#     sizeFreqDistMCCAll <- c(sum(metadata$country == "Germany") - sum(clusterStatsMCC$seqs), sizeFreqDistMCC)
...
#    cumPropMCC <- c(0, cumsum(lineageSizesMCC)/sum(lineageSizesMCC))
    ...
    stateInfo$sizeFreqDistMCC[[s]] <- sizeFreqDistMCC
    stateInfo$sizeBreaks[[s]] <- sizeBreaks
  }
```


```{r lineage-sizes, fig.width=7, fig.height=3, fig.cap="Distribution of detected importation lineages size."}

    

  # for (s in stateInfo$stateOrder) { #1:nrow(stateFiles)
    s <- stateInfo$stateOrder
  # for (s in 1:1) { #

   state <- stateFiles$state[s]
   clusterStatsMCC <- stateInfo$clusterStatsMCC[[s]] 
   clusterSamplesMCC <- stateInfo$clusterSamplesMCC[[s]] 
   sizeFreqDistMCC <- stateInfo$sizeFreqDistMCC[[s]]
   sizeBreaks <- stateInfo$sizeBreaks[[s]]
   ...
   # par(mar=c(4,6,2,6), cex.axis=0.7, cex.lab=0.8, mgp=c(3,0.75,0), fig=c(0,1,0,1))
   par(mar=c(3,4,2,2), cex.axis=0.7, cex.lab=0.8, mgp=c(2,0.75,0))

   barplot2(c(sizeFreqDistMCC[1:n], sum(sizeFreqDistMCC[(n+1):length(sizeFreqDistMCC)])), 
            #names.arg = colnames(sizeFreqDistHPD),
            plot.ci = TRUE, ci.color = mPal(dePal$eng), 
            # ci.l = sizeFreqDistHPD[1, ], ci.u = sizeFreqDistHPD[3, ],
            ci.l = c(sizeFreqDistMCC[1:n], sum(sizeFreqDistMCC[(n+1):length(sizeFreqDistMCC)])), 
            ci.u = c(sizeFreqDistMCC[1:n], sum(sizeFreqDistMCC[(n+1):length(sizeFreqDistMCC)])),
            col=c(rep(mPal(col, 0.5),n), mPal(colOth, 0.5)),
            border=c(rep(mPal(col),n), mPal(colOth)),
            cex.names = 0.7, las=2, ylim=c(0,ymax), width=0.8, space=0.25, xpd=FALSE,
            ylab="No. of importation lineagges", 
            main="")
   
    title(xlab="importation lineage size", mgp = c(1.2, 1, 0))
    # text(x = 1:(n+1)-0.4, y = -0.06*ymax,
    #      labels = c(1:n, paste0(">", n)),
    #      # labels = colnames(sizeFreqDistHPD), 
    #      # labels = colnames(sizeFreqDistHPD), 
    #      cex=par("cex.axis"), srt=45, xpd=TRUE)
    axis(1, at= 1:(n+1)-0.4, 
         #y = -0.06*ymax,
         labels = c(1:n, paste0(">", n)),
         # labels = colnames(sizeFreqDistHPD), 
         # labels = colnames(sizeFreqDistHPD), 
         las = 2,
         cex=par("cex.axis"), srt=45, xpd=TRUE, mgp = c(1, 0.6, 0))
...
#            c(cumFreqDistHPD[3,], rev(cumFreqDistHPD[1,])), border=NA, col=mPal(ukPal$eng, 0.5))
 ...
 #   changeSlopeX = 2
    ...
    # cumFreqDistHPD <- cumsum(sizeFreqDistMCC)
#    cumFreqDistMCC  <- rev(cumsum(rev(sizeFreqDistMCC)))
    
    # Median 
#    lines(sizeBreaks[2:length(sizeBreaks)], cumFreqDistMCC, col=mPal(ukPal$eng))
    
    # MCC tree clustering
 #   lines(sizeBreaks[2:length(sizeBreaks)], cumFreqDistMCC, col=mPal(ukPal$sct), lty=1)
    # 
 #   abline(v = changeSlopeX * 10, col=mPal(dark$black, 0.5), lty=2)
    # 
...
```{r lineage-rank-complete, fig.width=8, fig.height=4, fig.cap="The percentage of `r states` genomes contained in transmission lineages, when ranked from biggest to smallest. The blue line and shading shows respectively the percentage of genomes in transmission lineages on the MCC trees and across the posterior tree distribution, respectively. The corresponding size of transmission lineages is shown in red on a logarithmic axis. The black dashed line and yellow shading show the 20% biggest transmission lineages on the MCC trees and across the posterior tree distribution, respectively."}
...
```{r lineage-durations, fig.width=5, fig.height=4, fig.cap="Distribution of `r states` transmission lineage sampling durations, aggregated by week. Blue bars show the number of transmission lineages that were observed over different durations in the MCC tree. Red bars show 95% HPD intervals for these numbers across the posterior tree distribution."}

#    ymax <- 400
#    
#    durationBreaks <- seq(0, 1000, by=7)
...
```{r tmrca-captions} 
    ...
    tmrca_density_cap <- paste0("The number of ", state, " transmission lineage TMRCAs on each date across the 2000 posterior trees (median = blue line, 50% HPD interval = dark blue shading, 95% HPD interval = light blue shading).")

    tmrca_mcc_hist_cap <- paste0("Histogram of lineage TMRCAs, coloured by lineage size. Inset: expanded view of the days prior to ", state, " lockdown. Left-hand arrow = collection date of the ", state, "’s first laboratory-confirmed case; right-hand arrow = collection date of the earliest ", state, " virus genome in our dataset.")
    
    tmrca_comparison_cap <- paste0("Comparison between the number of ", state, " transmission lineage TMRCAs on each date in the MCC trees (red line) and across the 2000 posterior trees (median = blue line, 95% HPD interval = blue shading). Unevenness in this distribution is mostly likely caused by the phylogenetic constraints imposed by the sequence sampling times.")
...
#    tmrcaBreaks  <- seq.Date(as.Date(startDate), as.Date(endDate), by="days")
#
...
```{r tmrca-mcc-hist, fig.width=9, fig.height=4, fig.cap = tmrca_mcc_hist_cap, eval=TRUE}
  ...
  for (s in 1:nrow(stateFiles)) {
    ...
    tmrcaBreaks <- seq.Date(min(clusterStatsMCC$tmrca_calendar, na.rm = TRUE), max(clusterStatsMCC$tmrca_calendar, na.rm = TRUE), by="1 days")
    # tmrcaBreaks  <- seq.Date(as.Date("2019-01-01"), as.Date("2021-06-24"), by="days")
    ...
    #rect(as.Date("2020-01-17")-startDate, 0, as.Date("2020-02-31")-startDate, ymax, lty=2)
    #rect(as.Date("2020-03-01"), 0, as.Date("2020-12-20"), ymax, lty=2)
...
# 		col=mPal(unlist(ukPal), 0.75), border=mPal(unlist(ukPal)), ymax=ymax)
#     
#     rect(0, 0, 100, ymax, xpd=TRUE)
    
    # points(x=(labelDates$oldestCase-as.Date("2020-03-19")), y=-0.09*ymax, pch=173, font=5, cex=1.5, xpd=TRUE, col="#000011")
    # points(x=(labelDates$oldestSeq-as.Date("2020-01-19")), y=0.12*ymax, pch=175, font=5, cex=1.5, xpd=TRUE, col="#111111")
    
    points(x=firstCase-as.Date("2020-01-01"), y=-0.5, pch=173, font=5, cex=1.5, xpd=TRUE, col="#FF0000")
    points(x=firstSeq-as.Date("2020-01-01"), y=0.5, pch=175, font=5, cex=1.5, xpd=TRUE, col="#FF0000")
  }
```
...
#
#    #tmrcaBreaks  <- seq.Date(as.Date("2019-12-01"), as.Date("2020-06-26"), by="days")
#    tmrcaBreaks  <- seq.Date(as.Date("2019-12-01"), as.Date(endDate), by="days")
#
...
```{r sizeduration-vs-tmrca, fig.width=5, fig.height=6, fig.cap="Scatterplots showing the relationship between (A) `r states` transmission lineage size and lineage TMRCA and between (B) `r states` transmission lineage sampling duration and lineage TMRCA. Pearson correlation coefficients, 95% CIs and p-values are shown in the top-right corners."} 
  ...
  for (s in 1:nrow(stateFiles)) {
    ...
    plotLineageScatter(clusterStatsMCC, stat1="tmrca", stat2="seqs", startDate = startDate, endDate = endDate, addLine = TRUE, ymax=max(clusterStatsMCC$seqs)+2, log=TRUE, label="A", 
                   ylab=paste("Transmission lineage size \nof", state))

    plotLineageScatter(clusterStatsMCC, stat1="tmrca", stat2="duration", startDate = startDate, endDate = endDate, addLine = TRUE, ymax=max(clusterStatsMCC$duration)+2, log=FALSE, label="B", 
...
```{r size-vs-duration, fig.width=5, fig.height=3, fig.cap="Scatterplot showing the strong relationship between `r states` transmission lineage size and sampling duration. The Pearson correlation coefficient, 95% CI and p-value are shown."}
  ...
  for (s in 1:nrow(stateFiles)) {
    ...
    plotScatter(clusterStatsMCC, stat1="duration", stat2="seqs", xmax=50, addLine = TRUE, ymax=max(clusterStatsMCC$seqs)+2, log=TRUE, plotCI = TRUE, 
                   ylab=paste("Transmission lineage size \nof", state), xlab="Duration of transmission\nlineage detection (days)")
  }        
...
```{r time-unsampled-data, eval=TRUE}
  ...
  for (s in 1:nrow(stateFiles)) {
    ...
    # endDate = as.Date("2021-04-24")
    #unseenBreaks <- seq.Date(as.Date("2020-02-10"), endDate+4, by="days")
    unseenBreaks <- seq.Date(startDate, endDate+6, by="7 days")
    #timeBreaks   <- seq(0, 147, by=7)
    timeBreaks   <- seq(0, 1000, by=7)
    timeUnsampled   <- c()
    timeDetected    <- c()
    ...
    for (i in 1:(length(unseenBreaks)-1)) {
      ...
      timeUnsampled <- rbind(timeUnsampled, rep(0, length(timeBreaks)-1))  
      
      timeDetected  <- rbind(timeDetected, rep(0, length(timeBreaks)-1))  
      }
      rn <- c(rn, format.Date(unseenBreaks[i], format="%Y-%m-%d"))
    }
    
    #rownames(timeUnsampled) <- rownames(timeDetected) <- format.Date(unseenBreaks[1:(length(unseenBreaks)-1)], format="%Y-%m-%d")
    rownames(timeUnsampled) <- rownames(timeDetected) <- rn
...
#    genomeRate$raw   <- genomesPerDay[8:(length(genomesPerDay))]
  ...
  stateFiles$timeUnsampledProps[[s]] = timeUnsampledProps
  stateFiles$unseenBreaks[[s]] = unseenBreaks
  stateFiles$timeDetectedProps[[s]] = timeDetectedProps
  stateFiles$timeUnsampled[[s]] = timeUnsampled
  stateFiles$timeDetected[[s]] = timeDetected
  stateFiles$extinctionRate[[s]] = extinctionRate
  stateFiles$detectionRate[[s]] = detectionRate
  stateFiles$genomeRate[[s]] = genomeRate
  stateFiles$genomesPerDay[[s]] = genomesPerDay
...
# WHAT WAS IT?
  ...
  #   clusterSamplesMCC <- stateInfo$clusterSamplesMCC[[s]] 
  # 
  # timeUnsampledProps <- stateFiles$timeUnsampledProps[[s]]
  # unseenBreaks <- stateFiles$unseenBreaks[[s]]
  # timeDetectedProps <- stateFiles$timeDetectedProps[[s]]
  # timeUnsampled <- stateFiles$timeUnsampled[[s]]
  # timeDetected <- stateFiles$timeDetected[[s]] 
  # extinctionRate <- stateFiles$extinctionRate[[s]] 
  # detectionRate <- stateFiles$detectionRate[[s]] 
  # genomeRate <- stateFiles$genomeRate[[s]] 
  # genomesPerDay <- stateFiles$genomesPerDay[[s]] 
  # 
  #  par(mar=c(4,4,2.5,8), cex.axis=0.7, cex.lab=0.8, mgp=c(2,0.75,0))
  # 
  #  #plotDateGradient(timeUnsampledProps[,1:17], unseenBreaks, startDate="2020-03-01", endDate="2020-06-07",
  #  plotDateGradient(timeUnsampledProps[,1:65], unseenBreaks, startDate=unseenBreaks[1], endDate=unseenBreaks[length(unseenBreaks)-1],
  #                   barplot=FALSE, border="#00000044", lty=1, lwd=0.5, axes=TRUE,
  #                   ylab="Proportion of\ntransmission lineages",
  #                   palfn = inferno, alpha=0.75, direction=-1,
  #                   legend="Weeks since\nlast sampled", legendPos = c(0.93, 0.95, 0.3, 0.6), main=state)
  # 
  #  # Highlight 1 week and 4 week isoclines
  #  polygon(ymd(c(startDate, rownames(timeUnsampledProps), rownames(timeUnsampledProps)[nrow(timeUnsampledProps)], startDate)), c(0, timeUnsampledProps$Less_1w, 0, 0),         border="#000000CC")
  #  polygon(ymd(c(startDate, rownames(timeUnsampledProps), rownames(timeUnsampledProps)[nrow(timeUnsampledProps)], startDate)), c(0, rowSums(timeUnsampledProps[, 1:4]), 0, 0), border="#FF0000CC")
  # 
  #  # Newly detected lineages
  #  polygon(ymd(c(startDate, rownames(timeDetectedProps), startDate)), c(100, timeDetectedProps$Less_1w, 0), angle = 45, density = 15, lty=2, border="#000000CC", col="#000000CC")
  # 
  #  # Mark lockdown
...
```{r time-unsampled-rates,  fig.width=7, fig.height=3, fig.cap="Red line=daily rate of detecting new transmission lineages. Blue line=rate at which lineages have not been observed for >4 weeks, shading=95% HPD across the posterior tree distribution.", eval=TRUE}
   ...
   # ymax <- 0.25
   # 
   # plotShadedAxes(xlim=c(ymd("2020-03-01"), ymd("2021-03-24")), ylim=c(0, ymax), yaxs='i', ylab="Transmission lineages / genome", axes=TRUE)
   # 
...
```{r time-unsampled-absolute,  fig.width=7, fig.height=3, fig.cap="Trends through time in the detection of `r states` transmission lineages (absolute values). For each day, all lineages detected up to that day are coloured by the time since the transmission lineage was last sampled. Isoclines correspond to weeks. Shaded area=transmission lineages that were first sampled <1 week ago.", eval=TRUE}
  ...
  genomesPerDay <- stateFiles$genomesPerDay[[s]] 
   ...
   plotDateGradient(timeUnsampled, unseenBreaks, startDate=unseenBreaks[1], endDate=unseenBreaks[length(unseenBreaks)-1],
#   plotDateGradient(timeUnsampled, unseenBreaks, startDate="2020-03-01", endDate="2021-03-24",
                    barplot=FALSE, border="#00000044", axes=TRUE, normalise = FALSE,
                    ylab="No. of\ntransmission lineages",
                    palfn = inferno, alpha=0.75, direction=-1,
                    legend="Weeks since\nlast sampled", legendPos = c(0.93, 0.95, 0.3, 0.6))

   polygon(ymd(c(startDate, rownames(timeDetected))), c(0, timeDetected$Less_1w), angle = 45, density = 15, lty=2, border="#000000CC", col="#000000CC")


   par(new=TRUE)

   ymax <- max(rowSums(timeUnsampled)) + 2
   plot(1, type='n', xlim=c(ymd(startDate), ymd(endDate)), ylim=c(0,ymax), axes=FALSE, xaxs='i', yaxs='i', xlab="", ylab="")
   axis(4, las=2)
   ...
   legend("bottom", horiz=FALSE, inset=c(0,1), bty='n', xpd=TRUE, ncol=2,
          col=c(mPal(ukPal$eng), mPal(ukPal$sct)), lwd=2,
          legend = c("Detecting new transmission lineages", "Lineages not observed >4 weeks"), title=paste("Rate of", state), cex=par("cex.lab"))
